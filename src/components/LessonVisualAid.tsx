...
const LESSON_INFOGRAPHIC_ASSET_PATHS = Object.fromEntries(
  LESSON_INFOGRAPHIC_SLUGS.map((slug) => [slug, `${LESSON_INFOGRAPHIC_BASE_PATH}/${slug}.webp`]),
...
// Fundo de espera/fallback do infográfico (fica atrás da imagem e aparece se
// ela falhar): base escura da marca com brilho no acento da linguagem.
const FallbackBackdrop = ({ tone }: { tone: VisualTone }) => (
  <svg viewBox="0 0 520 250" role="presentation" aria-hidden="true" className="h-full w-full">
    <rect width="520" height="250" rx="20" fill={tone.bg} />
    <circle cx="420" cy="40" r="150" fill={tone.accent} opacity="0.12" />
    <circle cx="70" cy="220" r="90" fill={tone.primary} opacity="0.08" />
  </svg>
);
    ...
    <figure className="mb-5 overflow-hidden rounded-xl border border-border bg-background shadow-sm sm:rounded-2xl">
      <div className={stacked ? "grid gap-0" : "grid gap-0 lg:grid-cols-[minmax(0,1.1fr)_minmax(260px,0.9fr)]"}>
        <div className="relative min-h-[190px] overflow-hidden bg-secondary/30 p-2 sm:min-h-[250px] sm:p-3">
          ...
          />
          <div className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-black/45 px-3 py-1.5 text-[11px] font-black text-white ring-1 ring-white/15 backdrop-blur sm:left-5 sm:top-5 sm:text-xs">
            <CourseIcon size={15} />
